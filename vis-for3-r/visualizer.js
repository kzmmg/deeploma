...
function fill_scene(pdb, camera, options) {
	...
	if (options.reg_plane) {
		let normal = vis_helpers.compute_normal(geometry.vertices)
		let plane = vis_helpers.make_plane(normal, bounding_box, options)
	...
	if (options.axis) {
		const axesHelper = new THREE.AxesHelper( options.axis_length || vis_constants.axis_default_length )
	...
	const ambient_light = new THREE.AmbientLight(compute_ambient_light(options))
...
function compute_background_color(options) {
	return compute_color(options.light)
}

function compute_ambient_light(options) {
...
function init(options) {
	...
	return [ renderer, camera, camera_controls ]
}

function add_to_dom(element, renderer) {
...
function render(clock, renderer, camera, camera_controls, scene) {
...
export default function visualizer(element, pdb, options) {
	try {
		...
		const res = init(options)
		
		const renderer = res[0]
		const camera = res[1]
