...
const cwdfy = (pref, ext) => {
	return process.cwd() + "/" + pref + "." + ext
}

const serialize_to_json = (filename_prefix, data) => {
	return fs.writeFileSync(cwdfy(filename_prefix, "json"), JSON.stringify(data, null, 2))
}
...
const encode_pdb = (filename_prefix, molecule) => {
	let filename = cwdfy(filename_prefix, "pdb")
...
const serialize_to_simplified_pdb = (filename, data) => {
	encode_pdb(filename, data)
}

module.exports = {
	serialize_to_json: serialize_to_json,
	serialize_to_simplified_pdb: serialize_to_simplified_pdb
}